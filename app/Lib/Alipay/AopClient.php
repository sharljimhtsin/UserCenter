...
class AopClient
{
    ...
    public function sdkExecute(AlipayTradeWapPayRequest $request)
    {
    ...
    /*
        ...
        auther:笙默
    */
    public function pageExecute(AlipayTradeWapPayRequest $request, $httpmethod = "POST")
    {
    ...
    public function execute(AlipayTradeWapPayRequest $request, $authToken = null, $appInfoAuthtoken = null)
    {
    ...
    public function rsaEncrypt($data, $rsaPublicKeyPem, $charset)
    {
        ...
        foreach ($blocks as $n => $block) {
            if (!openssl_public_encrypt($block, $text, $res)) {
                echo "<br/>" . openssl_error_string() . "<br/>";
            }
            array_push($encodes, $text);
        }
        $longText = implode(",", $encodes);
        return $longText;
    }
    ...
    function parserResponseSubCode(AlipayTradeWapPayRequest $request, $responseContent, $respObject, $format)
    {
    ...
    function parserJSONSignSource(AlipayTradeWapPayRequest $request, $responseContent)
    {
        $apiName = $request->getApiMethodName();
        $rootNodeName = str_replace(".", "_", $apiName) . $this->RESPONSE_SUFFIX;
        $rootIndex = strpos($responseContent, $rootNodeName);
    ...
    function parserXMLSignSource(AlipayTradeWapPayRequest $request, $responseContent)
    {
        $apiName = $request->getApiMethodName();
        $rootNodeName = str_replace(".", "_", $apiName) . $this->RESPONSE_SUFFIX;
        $rootIndex = strpos($responseContent, $rootNodeName);
    ...
    private function parserEncryptJSONSignSource(AlipayTradeWapPayRequest $request, $responseContent)
    {
    ...
    private function parserEncryptXMLSignSource(AlipayTradeWapPayRequest $request, $responseContent)
    {
